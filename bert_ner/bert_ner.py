...
class BERT_NER(BaseEstimator, ClassifierMixin):
    ...
    def tokenize_all(self, X: Union[list, tuple, np.array],
        ...
        if y is None:
            ...
            for sample_idx in range(n_samples):
                ...
                labels_IDs = dict()
                label_ID = 1
                for ne_type in y[sample_idx]:
                    class_id = self.classes_list_.index(ne_type) + 1
                    for ne_bounds in y[sample_idx][ne_type]:
                        for char_idx in range(ne_bounds[0], ne_bounds[1]):
                            indices_of_named_entities[char_idx] = label_ID
                        labels_IDs[label_ID] = class_id
                        label_ID += 1
                y_tokenized[sample_idx] = self.detect_token_labels(bounds_of_tokens, indices_of_named_entities,
                                                                   labels_IDs, self.max_seq_length)
                if len(tokenized_text) > (self.max_seq_length - 2):
                    tokenized_text = tokenized_text[:(self.max_seq_length - 2)]
                tokenized_text = ['[CLS]'] + tokenized_text + ['[SEP]']
                token_IDs = self.tokenizer_.convert_tokens_to_ids(tokenized_text)
    ...
    def calc_similarity_between_entities(gold_entity: Tuple[int, int], predicted_entity: Tuple[int, int]) -> \
        ...
        if gold_entity[1] <= predicted_entity[0]:
            res = gold_entity[1] - predicted_entity[0] - 1.0
            tp = 0
            fp = 0
            fn = 0
        elif predicted_entity[1] <= gold_entity[0]:
            res = predicted_entity[1] - gold_entity[0] - 1.0
            tp = 0
            fp = 0
            fn = 0
        else:
    ...
    def calculate_prediction_quality(true_entities: Union[list, tuple, np.array],
                                     ...
                                     classes_list: tuple) -> Tuple[float, float]:
        Q_total = 0
        S_total = 0
        T_total = 0
        f1_macro = 0
        ...
        for ne_class in classes_list:
            ...
            T = 0
            for sample_idx in range(n_samples):
                if (ne_class in true_entities[sample_idx]) and (ne_class in predicted_entities[sample_idx]):
                    pairs_list_ = []
                    for true_bounds in true_entities[sample_idx][ne_class]:
                        for predicted_bounds in predicted_entities[sample_idx][ne_class]:
                            pairs_list_.append(
                                (
                                    true_bounds,
                                    predicted_bounds,
                                    BERT_NER.calc_similarity_between_entities(true_bounds, predicted_bounds)
                                )
                            )
                    pairs_list_.sort(key=lambda it: (-it[2][0], -it[2][1], -it[2][2], -it[2][3]))
                    pairs_list = [pairs_list_[0]]
                    used_true_idx = {pairs_list_[0][0]}
                    used_pred_idx = {pairs_list_[0][1]}
                    for cur in pairs_list_[1:]:
                        if (cur[0] in used_true_idx) or (cur[1] in used_pred_idx):
                            continue
                        pairs_list.append(cur)
                        used_true_idx.add(cur[0])
                        used_pred_idx.add(cur[1])
                    del used_true_idx, used_pred_idx, pairs_list_
                elif ne_class in true_entities[sample_idx]:
                    pairs_list = [(ne_bounds, None, (0.0, 0, 0, 0))
                                  for ne_bounds in true_entities[sample_idx][ne_class]]
                elif ne_class in predicted_entities[sample_idx]:
                    pairs_list = [(None, ne_bounds, (0.0, 0, 0, 0))
                                  for ne_bounds in predicted_entities[sample_idx][ne_class]]
                else:
                    pairs_list = None
                if pairs_list is not None:
                    for cur in pairs_list:
                        Q += cur[2][0]
            ...
            if (precision + recall) > 0.0:
                ...
                f1_macro += f1
                n += 1
        precision = Q_total / float(T_total)
        recall = Q_total / float(S_total)
        if (precision + recall) > 0.0:
            f1_micro = 2 * precision * recall / (precision + recall)
        else:
            f1_micro = 0.0
        if n > 0:
            f1_macro = f1_macro / float(n)
        else:
            f1_macro = 0.0
